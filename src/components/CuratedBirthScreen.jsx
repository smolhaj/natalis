...
const STABILITY_OPTIONS = [
  { value: 'secure',    label: 'Secure',    desc: 'Stable, privileged, loving household', color: '#3f5670' },
  { value: 'stable',    label: 'Stable',    desc: 'Solid, modest, functional family', color: '#3f6146' },
  { value: 'struggling', label: 'Struggling', desc: 'Real hardship, but holding together', color: '#8a6635' },
  { value: 'unstable',  label: 'Unstable',  desc: 'Difficult, unpredictable circumstances', color: '#8c3a2e' },
]
        ...
        <div className="flex gap-2 justify-center">
          {[1,2,3,4].map(s => (
            <div key={s} className="w-2 h-2 rounded-full transition-all"
              style={{ background: s <= step ? '#3f5670' : '#d1d1d6' }} />
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-3">
              ...
              <div className="max-h-[52vh] overflow-y-auto space-y-1 pr-1">
                ...
                {filteredCountries.map(c => (
                  <button
                    ...
                    style={{
                      background: country === c.name ? '#3f5670' : '#f4f2ed',
                      color: country === c.name ? 'white' : '#1c1a16',
                    }}
                  >
            ...
            <button
              ...
              style={{ background: canProceed(1) ? '#3f5670' : '#d0c9b9' }}
            >
              Next →
            </button>
          </div>
          ...
          <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {['male', 'female'].map(g => (
                    <button
                      ...
                      style={{
                        background: gender === g ? '#3f5670' : '#f4f2ed',
                        color: gender === g ? 'white' : '#7d766a',
                      }}
                    >
                      {g === 'male' ? '♂ Male' : '♀ Female'}
                    </button>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                style={{ background: canProceed(2) ? '#3f5670' : '#d0c9b9' }}
              >
                Next →
              </button>
            </div>
                ...
                <div className="space-y-1">
                  {RURAL_OPTIONS.map(opt => (
                    <button
                      ...
                      style={{
                        background: ruralUrban === opt.value ? '#3f5670' : '#f4f2ed',
                        color: ruralUrban === opt.value ? 'white' : '#1c1a16',
                      }}
                    >
                ...
                <div className="space-y-1">
                  {STABILITY_OPTIONS.map(opt => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-xl transition-all"
                      style={{
                        background: familyStability === opt.value ? opt.color : '#f4f2ed',
                        color: familyStability === opt.value ? 'white' : '#1c1a16',
                      }}
                    >
                      <span className="text-sm font-semibold">{opt.label}</span>
                      <span className="text-xs opacity-60 ml-2">{opt.desc}</span>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {religions.map(r => (
                      <button
                        ...
                        style={{
                          background: religion === r ? '#3f5670' : '#f4f2ed',
                          color: religion === r ? 'white' : '#403b33',
                        }}
                      >
                        {RELIGION_LABELS[r] ?? r}
                      </button>
