...
// A place id's name carries a disambiguator for the benefit of whoever is
// reading places.js — "Rural Bahia (sertão)", "Rural Moldova (Romania)",
...
const placeName = (G, fallback = 'here') => {
  ...
  return stripped.replace(/^Rural\s+/, '') || fallback
}

// Same problem one level down, and worse: the neighbourhood lists hold a few
// category nouns rather than names — Egypt's informal tier is "Ashwa'iyyat",
// which is simply the Arabic word for informal settlements. Those belong in
// places.js, which is owned elsewhere; until then a caller can ask whether the
// string is usable as a name before putting it in a sentence.
const CATEGORY_NOUNS = ['Ashwa\'iyyat', 'Akwakh al-ummah']
const neighbourhoodName = (G) => {
  const nbr = G.neighborhood
  if (!nbr || CATEGORY_NOUNS.includes(nbr)) return null
  return nbr
}
...
export const PLACES_EVENTS = [
...

  {
    ...
    text: (G) => {
      const nbr = neighbourhoodName(G) ?? placeName(G, 'this place')
...

  {
    ...
    text: (G) => {
      const nbr = neighbourhoodName(G) ?? 'the settlement'
      return `A notice goes up. The land ${nbr} stands on has been sold to a developer, or rezoned, or the city wants to clear it for something else. The language is bureaucratic. The meaning is: leave. People gather to discuss what to do. Some have been here thirty years.`
    },
    choices: [
      {
...

  {
    ...
    // The body described a city neighbourhood and the header printed a data
    // label: "Perto do mercado in Rural Bahia (sertão): the bus stops at 6am."
    // The village is a different place and needs the sentence that fits it.
    text: (G) => {
      const nbr = neighbourhoodName(G)
      const where = placeName(G, 'here')
      if (G.place?.type === 'rural') {
        return `${where}: the day starts before it is light and everyone is doing a version of the same work, so everyone knows roughly what everyone else has. The shop sells what the shop sells. The ambitions here are concrete: the roof done properly, an animal that does not have to be sold, a child kept in school past the year when children stop. Nothing abstract. Nothing pretending.`
      }
      const head = nbr ? `${nbr} in ${where}` : where
      return `${head}: the bus stops at six in the morning full of people going to work. The shops open at seven. On the days off the street fills up. Everyone knows roughly what everyone else earns. The ambitions here are concrete: a better flat, a car, a child at the good school. Nothing abstract. Nothing pretending.`
    },
    choices: null,
    effect: (p) => { p.m += 2 },
  },

  {
    ...
    text: (G) => {
      const nbr = neighbourhoodName(G) ?? 'this part of the city'
...

  {
    ...
    text: (G) => {
      const old = neighbourhoodName(G) ?? 'the old place'
...

  {
    ...
    text: (G) => {
      const city = placeName(G, 'this city')
...

  {
    ...
    text: (G) => {
      const place = placeName(G, 'this place')
      const nbr = neighbourhoodName(G) ?? ''
