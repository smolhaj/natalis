...
export const EVENTS_SONDER_51 = [
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s51b,
    text: 'An alarm did not go off. You wake at the right time anyway, five minutes before it would have sounded, as if the body has its own contract with the hour. The alarm would have been unnecessary. The body did not know this when it made the arrangement.',
    choices: null,
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s51c,
    text: 'The flat is the first flat that is only yours. Not a room in someone else\'s house, not shared. The key in the door. The silence of a space that is only yours. For a week everything in it feels provisional. Then you buy a plant and it begins to feel like a place.',
    choices: null,
...

{
    ...
    weight: 2,
    when: (G) => !G.mem?.s51h,
    text: 'It is going well and does not feel like it is going well, because you are inside it and cannot see the whole. Later you will see the whole and understand that this was the good period. For now it feels like effort and uncertainty and the sense that you are not doing enough, which is how the good periods often feel from inside them.',
    choices: null,
...

{
    ...
    weight: 2,
    when: (G) => !G.mem?.s51l,
    text: 'You know how to make one meal without consulting anything. The sequence is in your hands before you think about it: the timing, the order, the moment when you add the thing you add. You learned it by watching rather than by being taught. You would have difficulty writing the recipe down, because the recipe is in your body, not in your head.',
    choices: null,
...

{
    ...
    weight: 2,
    when: (G) => !G.mem?.s51x,
    text: 'One object is yours alone — not a particularly valuable object, simply yours. You carry it or keep it somewhere specific. If it were lost you would feel the loss before you could name what was lost. The attachment to it is not rational and is not nothing.',
    choices: null,
...

{
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s51ab,
    text: 'Adults are laughing in another room. Not seeing the joke, just hearing the laughter. The sound has a quality when you are not in the room — warm and separate, the adults doing something adults do that is not available to you yet. You listen from outside it. You will have access eventually. Right now you are listening to the category of things you do not yet have access to.',
    choices: null,
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s51ac,
    text: 'A compliment arrives out of context from someone you have not seen in years. A message, a chance encounter, a reference to something you did that you have forgotten. The person has held this for long enough to offer it. You receive it with some difficulty, which is information about your relationship to receiving.',
    choices: null,
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s51ad,
    text: 'You carry something you have never put down: not a grief exactly, not a regret, but something that has been in the background of every year for as long as you can remember. You have become so accustomed to it that you sometimes forget it is there. Then something small reminds you and it is there, exactly as it was. It has not gotten heavier. It has also not gotten lighter. You have simply gotten used to its weight.',
    choices: null,
