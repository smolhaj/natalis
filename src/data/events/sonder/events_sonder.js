...
import { hasBooks, hasBus, hasCafe, hasCar, hasClock, hasElectricity, hasHealthcare, hasMetro, hasPhone, hasPhotographs, hasRunningWater, hasSupermarket, hasWeekend, isHotCountry, isLiterate, isPoor, isUrban, wentToSchool, worksInOffice } from './_sonderGuards.js'
...
export const SONDER_EVENTS = [
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasElectricity(G) && (G.age >= 18 && G.age <= 30 && !G.mem?.sonderLitWindow),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => isUrban(G) && (G.age >= 32 && G.age <= 55 && !G.mem?.sonderChildrenBelow),
...

  {
    ...
    text: 'You have developed a morning ritual. It is small and private and probably unremarkable to anyone who is not you: the order of things, the quality of the first hour. You did not decide to develop it. It assembled itself. You notice, this year, that you look forward to it.',
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasHealthcare(G) && (G.age >= 28 && G.age <= 58 && !G.mem?.sonderHospitalCorridor),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasCafe(G) && (G.age >= 30 && G.age <= 55 && !G.mem?.sonderCoupleNotSpeaking),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasCar(G) && (G.age >= 28 && G.age <= 52 && !G.mem?.sonderChildWaveWindow),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasHealthcare(G) && (G.age >= 55 && !G.mem?.sonderHospitalRooftop),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => isLiterate(G) && (G.age >= 58 && !G.mem?.sonderWeddingPhoto),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => wentToSchool(G) && (G.age >= 32 && G.age <= 55 && !G.mem?.sonderChildHomework),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasBus(G) && (G.age >= 28 && G.age <= 60 && !G.mem?.sonderPrayerBeads),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => isUrban(G) && (G.age >= 18 && G.age <= 35 && !G.mem?.mundaneShortcutLearned),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => worksInOffice(G) && (G.age >= 28 && G.age <= 55 && G.career && !G.mem?.mundaneWorkAcquaintanceLeft),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasWeekend(G) && (G.age >= 18 && G.age <= 40 && G.career && !G.mem?.mundaneFridayFeeling),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasPhotographs(G) && (G.age >= 58 && !G.mem?.mundanePhotographUnseen),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => wentToSchool(G) && (G.age >= 30 && G.age <= 50 && G.children?.length > 0 && !G.mem?.mundaneSchoolRunYears),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => wentToSchool(G) && (G.career?.field === 'education' && G.age >= 28 && !G.mem?.mundaneTeacherJune),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasHealthcare(G) && (G.career?.field === 'healthcare' && G.age >= 22 && G.age <= 45 && !G.mem?.mundaneNurseHandover),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => (G.career?.id === 'soldier' || G.flags.has('deployed_to_conflict') || G.flags.has('combat_veteran') || G.flags.has('returned_veteran')) && G.age >= 18 && G.age <= 40 && !G.mem?.mundaneSoldierDowntime,
...

  {
    ...
    phase: 'childhood',
    weight: 2,
    when: (G) => wentToSchool(G) && (G.age >= 6 && G.age <= 14 && !G.mem?.mundaneDeskNeighbour),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasClock(G) && (G.age >= 60 && !G.career && !G.mem?.mundaneRetirementMorning),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasWeekend(G) && (G.age >= 65 && !G.mem?.mundaneLongNap),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasHealthcare(G) && (G.age >= 60 && !G.mem?.mundaneDoctorRelationship),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => isUrban(G) && (G.character?.country?.archetype === 'post_soviet' && G.age >= 25 && G.age <= 62 && !G.mem?.sonderApartmentCleaner),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => isUrban(G) && (G.character?.religion?.startsWith('muslim') && G.age >= 28 && G.age <= 65 && !G.mem?.sonderFridayPrayerCourtyard),
...

  {
    ...
    phase: 'adolescence',
    weight: 2,
    when: (G) => hasPhotographs(G) && (G.age >= 13 && !G.mem?.sonderParentOldPhoto),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasPhotographs(G) && (G.career && G.age >= 36 && !G.mem?.sonderColleagueDeskPhoto),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => !['secular', 'atheist'].includes(G.religion) && (!G.mem?.sonderChurchBetweenServices),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasHealthcare(G) && (G.age >= 35 && !G.mem?.sonderHospitalWardNight),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasPhone(G) && (G.age >= 18 && G.age <= 26 && !G.mem?.sonderFlatmatePhoneWall),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => isUrban(G) && (G.age >= 34 && !G.mem?.sonderNeighbourLightSameHour),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasHealthcare(G) && (G.age >= 60 && !G.mem?.sonderHospitalOrdinaryWork),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasHealthcare(G) && (G.age >= 38 && !G.mem?.sonderOldManHospitalCorridor),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasCafe(G) && (!G.mem?.sonderCoupleArgumentPublic),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasBooks(G) && (!G.mem?.sonderLibraryAbsorbedReader),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasSupermarket(G) && (G.age >= 42 && !G.mem?.sonderWidowShoppingForOne),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => (G.currentCountry?.name === 'Canada' || G.currentCountry?.name === 'Germany' || G.currentCountry?.name === 'Russia' || G.currentCountry?.name === 'Norway' || G.currentCountry?.name === 'Poland' || G.currentCountry?.name === 'Ukraine' || G.currentCountry?.name === 'Hungary' || G.currentCountry?.name === 'Romania') && G.currentYear >= 1980 && !G.mem?.sonderFirstSnowWitness,
...

{
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => (G.religion === 'sikh' || G.character?.country?.name === 'India' || G.currentCountry?.name === 'Canada' || G.currentCountry?.name === 'United Kingdom') && !G.mem?.sonderSikhLangarServers,
...

  {
    ...
    text: 'The hagwon bus drops children home at ten-thirty in the evening. They get off with their backpacks, faces blank with the tiredness of children who have been studying for fourteen hours. They do not complain. You are not sure whether to call this diligence or something else.',
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasPhotographs(G) && ((G.flags.has('ofw_hongkong') || (G.character?.country?.name === 'Philippines' && G.flags.has('ofw_worker'))) && !G.mem?.sonderOfwVictoriaPark),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => isUrban(G) && ((G.character?.country?.name === 'Jordan' || G.character?.country?.name === 'Saudi Arabia') && !G.mem?.sonderBedouinTentCityEdge),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => worksInOffice(G) && (G.character?.gender === 'female' && G.career && G.age >= 22 && !G.mem?.sonderYoungWomanIdeaCredited),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => wentToSchool(G) && (G.character?.gender === 'male' && G.children?.length > 0 && G.age >= 35 && !G.mem?.sonderFathersSchoolGate),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasMetro(G) && ((G.character?.country?.name === 'South Africa' || G.character?.country?.name === 'Zambia' || G.character?.country?.name === 'Chile' || G.character?.country?.name === 'Bolivia' || G.character?.country?.name === 'Colombia') && !G.mem?.sonderMineShiftSurfaces),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasSupermarket(G) && (G.age >= 58 && !G.mem?.sonderDementiaCoupleSuper),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasCafe(G) && (G.age >= 42 && !G.mem?.sonderCarerParentPublic),
  ...
  // ── FIFTH EXPANSION BATCH: LIFE SITUATIONS, MISSING COUNTRIES, GENDER, RELIGION ──

  // LIFE SITUATIONS

  {
    id: 'sonder_man_tears_bench',
    phase: 'midlife',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 32 && !G.mem?.sonderManTearsBench,
    text: 'A man sitting on a bench with tears running down his face, making no sound and no attempt to wipe them. People passing give him a wide berth without looking directly. You look. You do not know what it cost him to be here like this in public. Whatever it was, the bench is doing its job.',
    effect: (p) => { p.e += 2; p.setMem('sonderManTearsBench', true); },
  },

  {
    id: 'sonder_office_box_leaving',
    phase: 'midlife',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.career && G.ruralUrban === 'urban' && G.currentYear >= 1970 && !G.mem?.sonderOfficeBoxLeaving,
    text: 'Someone coming out of a building with a box of their things — the plant, the mug, the framed photograph. They walk to the car. They put the box in the boot carefully. They sit in the driver\'s seat for a long time before starting the engine. Whatever just ended was not only a job.',
    effect: (p) => { p.e += 2; p.r += 1; p.setMem('sonderOfficeBoxLeaving', true); },
  },

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasHealthcare(G) && (G.age >= 24 && G.age <= 38 && !G.mem?.sonderNewParentsPavement),
    text: 'A couple outside a hospital with an infant that is clearly less than a week old. They are standing on the pavement looking at the street with the expression of people who have crossed a border and cannot go back. The world is moving at its normal speed around them. They are moving at a different speed.',
    effect: (p) => { p.e += 2; p.m += 1; p.setMem('sonderNewParentsPavement', true); },
  },

  {
    id: 'sonder_school_gate_first_day',
    phase: 'childhood',
    weight: 2,
    when: (G) => wentToSchool(G) && (!G.mem?.sonderSchoolGateFirstDay),
    text: 'A parent at the school gate after the first day of school — watching the door that closed behind their child, then not watching it, then watching it again. The child is fine. The parent will also be fine. Neither of them knows this yet and the not-knowing is visible.',
    effect: (p) => { p.m += 2; p.setMem('sonderSchoolGateFirstDay', true); },
  },

  {
    id: 'sonder_good_news_phone_public',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
    when: (G) => hasPhone(G) && (!G.mem?.sonderGoodNewsPhonePublic),
  ...
  {
    id: 'sonder_graduation_family_alignment',
    phase: 'young_adult',
    weight: 2,
  ...
  {
    id: 'sonder_birthday_alone_small_gesture',
    phase: 'midlife',
    weight: 2,
    when: (G) => G.age >= 36 && !G.mem?.sonderBirthdayAloneSmall,
    text: 'You notice someone in a café who has ordered a slice of cake — not with anyone, not waiting for anyone. There is one candle in it, not lit. They eat it slowly and they are neither happy nor sad about it. They are acknowledging something, to themselves, for themselves. You look away.',
    effect: (p) => { p.e += 2; p.setMem('sonderBirthdayAloneSmall', true); },
  },

  {
    id: 'sonder_courthouse_afterwards',
    phase: 'midlife',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 30 && !G.mem?.sonderCourthouseAfterwards,
    text: 'A person outside a courthouse, alone, standing very still. Not upset, or past upset. Whatever was decided inside is now a fact and the fact is new and they have not yet integrated it into their understanding of how things are. They are standing with the new fact before they carry it anywhere.',
    effect: (p) => { p.e += 2; p.setMem('sonderCourthouseAfterwards', true); },
  },
  ...
  {
    id: 'sonder_veteran_memorial_alone',
    phase: 'late_life',
    weight: 2,
    when: (G) => G.age >= 55 && G.currentYear >= 1950 && !G.mem?.sonderVeteranMemorialAlone,
    text: 'At the memorial at seven in the morning, one man. He is in his eighties and he has brought flowers. He stands there longer than the flowers require. He came at this hour so that he would be alone, which means that you, having come at this hour for your own reasons, are an intrusion. You leave him to it.',
    effect: (p) => { p.e += 2; p.r += 1; p.setMem('sonderVeteranMemorialAlone', true); },
  },

  {
    id: 'sonder_homeless_morning_arrangement',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
  ...
  {
    id: 'sonder_child_field_school_age',
    phase: 'childhood',
    weight: 2,
    when: (G) => (G.character?.country?.archetype === 'subsaharan' || G.character?.country?.archetype === 'developing_unstable') && G.currentYear >= 1960 && !G.mem?.sonderChildFieldSchoolAge,
    text: 'A child of perhaps eight working in a field at a time when children should be in school. You know the school and the field are not in competition with each other — they are in competition with the family\'s immediate need, which has won this round. The child is working with complete attention. The field will not wait.',
    effect: (p) => { p.e += 2; p.r += 2; p.setMem('sonderChildFieldSchoolAge', true); },
  },

  {
    id: 'sonder_florist_visit_grave',
    phase: 'midlife',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 35 && !G.mem?.sonderFloristVisitGrave,
    text: 'Someone buying flowers from the florist who is clearly going to a grave. Not a bouquet — a arrangement chosen with the knowledge of what someone liked. You cannot see this in the flowers, but you can see it in how they are chosen. The transaction is quiet on both sides.',
    effect: (p) => { p.e += 2; p.r += 1; p.setMem('sonderFloristVisitGrave', true); },
  },

  // SPECIFIC COUNTRIES / CULTURES

  {
    id: 'sonder_istanbul_teahouse_hours',
    phase: 'young_adult',
    weight: 2,
    when: (G) => (G.character?.country?.name === 'Turkey' || G.currentCountry?.name === 'Turkey') && !G.mem?.sonderIstanbulTeahouseHours,
    text: 'The tea house: the same men at the same table at the same hour they have occupied for years. The tea comes without ordering. The backgammon board comes out and the game takes two hours. No one is in a hurry. No one is wasting time either. The time is being spent on the thing that the time is for.',
    effect: (p) => { p.e += 2; p.setMem('sonderIstanbulTeahouseHours', true); },
  },

  {
    id: 'sonder_iran_threshold_adjustment',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
    when: (G) => (G.character?.country?.name === 'Iran' || G.currentCountry?.name === 'Iran') && G.currentYear >= 1980 && !G.mem?.sonderIranThresholdAdjustment,
    text: 'At the building entrance, a woman adjusts her hijab and, at the same time, something in her face. The outside version settling into place before she goes out. Inside: something more relaxed. Outside: the required posture. The transition takes about two seconds and is completely practiced.',
    effect: (p) => { p.e += 2; p.setMem('sonderIranThresholdAdjustment', true); },
  },

  {
    id: 'sonder_poland_market_grandmother',
    phase: 'midlife',
    weight: 2,
    when: (G) => (G.character?.country?.name === 'Poland' || G.character?.country?.name === 'Hungary' || G.character?.country?.name === 'Czech Republic' || G.character?.country?.name === 'Romania') && !G.mem?.sonderPolandMarketGrandmother,
    text: 'The grandmother at her market stall: the prices have changed so many times under so many systems that she holds them all in memory — what bread cost under communism, what it cost after, what it costs now. She quotes today\'s price without expression. She knows what it would have been.',
    effect: (p) => { p.e += 2; p.setMem('sonderPolandMarketGrandmother', true); },
  },

  {
    id: 'sonder_senegal_tailor_tree',
    phase: 'young_adult',
    weight: 2,
    when: (G) => G.character?.country?.name === 'Senegal' && !G.mem?.sonderSenegalTailorTree,
    text: 'A tailor sewing under a tree on the street, his machine run by a foot pedal. The clients come and stand nearby, and the standing nearby is how you indicate you are waiting. No one is impatient. The tailor works without looking up. He knows the weight of being waited on and does not let it rush him.',
    effect: (p) => { p.e += 2; p.setMem('sonderSenegalTailorTree', true); },
  },

  {
    id: 'sonder_buenos_aires_cafe_twenty_years',
    phase: 'midlife',
    weight: 2,
    when: (G) => G.character?.country?.name === 'Argentina' && G.currentYear >= 1970 && !G.mem?.sonderBuenosAiresCafe20Years,
    text: 'The regular at the corner table who has been at this table for twenty years. Everyone knows this about him. He reads the paper, he drinks his cortado, he leaves. Waiters retire and are replaced and the new ones are told about him. The table has become his in the way that no lease can convey.',
    effect: (p) => { p.e += 2; p.setMem('sonderBuenosAiresCafe20Years', true); },
  },

  {
    id: 'sonder_thailand_spirit_house',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.character?.country?.name === 'Thailand' || G.character?.country?.name === 'Cambodia' || G.character?.country?.name === 'Laos' && !G.mem?.sonderThailandSpiritHouse,
    text: 'Someone refreshing the offerings at a spirit house — the flowers, the incense, the small figures arranged just so. They do it with the deliberateness of someone who believes in what they are doing, which means the doing is done correctly. It takes five minutes. It has been done every morning for years.',
    effect: (p) => { p.e += 1; p.setMem('sonderThailandSpiritHouse', true); },
  },

  {
    id: 'sonder_ghana_chief_protocol',
    phase: 'midlife',
    weight: 2,
    when: (G) => G.character?.country?.name === 'Ghana' && !G.mem?.sonderGhanaChiefProtocol,
    text: 'At a gathering, the way the room organises itself around the chief\'s presence without discussion. The chair moved, the space cleared, the order of speaking deferred. This hierarchy is older than the constitution, older than the flag, older than the word for the nation. It operates without being enforced.',
    effect: (p) => { p.e += 2; p.setMem('sonderGhanaChiefProtocol', true); },
  },

  // GENDER / SEXUALITY

  {
    id: 'sonder_same_sex_couple_calibration',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.currentYear >= 1985 && G.character?.country?.archetype === 'wealthy_west' && !G.mem?.sonderSameSexCoupleCalibration,
    text: 'A couple walking — hand in hand or not quite hand in hand, depending on the street. You watch the small calibration: the hand taken and released based on something both of them are reading simultaneously. What they are reading is the street. The street changes block by block and their hands know it.',
    effect: (p) => { p.e += 2; p.setMem('sonderSameSexCoupleCalibration', true); },
  },

  {
    id: 'sonder_teenage_girl_street_posture',
    phase: 'adolescence',
    weight: 2,
    isGlimpse: true,
  ...
  {
    id: 'sonder_trans_person_window_check',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.currentYear >= 1990 && !G.mem?.sonderTransPersonWindowCheck,
    text: 'Someone checking their reflection in a shop window with the attention of a person who has specific stakes in what they see. Not vanity — assessment. They tilt slightly, they note something, they adjust slightly, they move on. The check takes longer than most people\'s and is more precise.',
    effect: (p) => { p.e += 2; p.setMem('sonderTransPersonWindowCheck', true); },
  },

  {
    id: 'sonder_hijab_western_city_attention',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
  ...
  {
    id: 'sonder_zoroastrian_fire_temple',
    phase: 'young_adult',
    weight: 2,
  ...
  {
    id: 'sonder_sufi_qawwali_gone',
    phase: 'young_adult',
    weight: 2,
    when: (G) => G.religion?.startsWith('muslim') && (G.character?.country?.name === 'Pakistan' || G.character?.country?.name === 'India') && !G.mem?.sonderSufiQawwaliGone,
    text: 'In the audience at the qawwali, one man has gone somewhere else. He is sitting but the sitting is only his body — his face has a quality you have seen in people who are very deep in something. The singers sing louder. He does not return until they stop.',
    effect: (p) => { p.e += 2; p.setMem('sonderSufiQawwaliGone', true); },
  },

  {
    id: 'sonder_rastafarian_reasoning',
    phase: 'young_adult',
    weight: 2,
    when: (G) => G.character?.country?.name === 'Jamaica' && !G.mem?.sonderRastafarianReasoning,
    text: 'Under a tree at the edge of the yard, four men reasoning: taking turns, following a thread through scripture and current events, each thought acknowledged before the next. The conversation is serious in the way of people who believe the conversation is the practice. They have been here two hours and will stay another two.',
    effect: (p) => { p.e += 2; p.setMem('sonderRastafarianReasoning', true); },
  },

  // LANGUAGE AND INTERPRETATION

  {
    id: 'sonder_interpreter_two_worlds',
    phase: 'young_adult',
    weight: 2,
  ...
  {
    id: 'sonder_adult_language_class_sounds',
    phase: 'young_adult',
    weight: 2,
    when: (G) => G.flags?.has?.('emigrated') && G.age >= 25 && !G.mem?.sonderAdultLanguageClassSounds,
    text: 'An adult in a language class trying to make a sound that does not exist in any language she already knows — her mouth attempting it, the teacher showing her where to place her tongue. She tries four times. She does not get it this week. She writes something in her notebook and does not stop trying.',
    effect: (p) => { p.e += 2; p.s += 1; p.setMem('sonderAdultLanguageClassSounds', true); },
  },

  {
    id: 'sonder_praying_transit',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
  ...
  {
    id: 'sonder_very_old_momentarily_lost',
    phase: 'late_life',
    weight: 2,
    when: (G) => isUrban(G) && (G.age >= 65 && !G.mem?.sonderVeryOldMomentarilyLost),
    text: 'A very old woman stops in the middle of the pavement for three or four seconds, with the look of someone who has temporarily lost her place. Then something returns — the head turns, the bearing adjusts, she walks on. You were the only person who noticed. She does not know you saw the three seconds. She has dealt with it and moved on.',
    effect: (p) => { p.e += 2; p.r += 1; p.setMem('sonderVeryOldMomentarilyLost', true); },
  },

  {
    id: 'sonder_maternity_ward_emergence',
    phase: 'young_adult',
    weight: 2,
    isGlimpse: true,
    when: (G) => hasHealthcare(G) && (!G.mem?.sonderMaternityWardEmergence),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => G.season === 'winter' && ((G.character?.country?.archetype === 'wealthy_west' || G.character?.country?.archetype === 'post_soviet') && !G.mem?.sonderFirstWarmDaySpring),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasBooks(G) && (G.age >= 38 && !G.mem?.sonderHouseEmptiedAfterDeath),
...

  {
    ...
    text: 'People leaving what is clearly a meeting of some kind of recovery programme — the handshakes, the quality of the conversation on the pavement after, the way they go their separate directions. Whatever is held together in that room is visible, briefly, in the quality of the leaving.',
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => ['Papua New Guinea', 'Samoa', 'Kiribati', 'Tuvalu', 'Marshall Islands', 'Fiji', 'New Zealand', 'Australia'].includes(G.currentCountry?.name) && !G.mem?.sonderPacificOceanReading,
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasCar(G) && (G.age >= 35 && !G.mem?.sonderManCryingAloneCar),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasCar(G) && (G.age >= 30 && !G.mem?.sonderArgumentWitnessedRestaurant),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasElectricity(G) && ((G.character?.country?.archetype === 'post_soviet' || G.character?.country?.name === 'Russia') && G.age >= 55 && !G.mem?.sonderSovietApartmentWindow),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => worksInOffice(G) && (G.character?.gender === 'female' && G.age >= 32 && G.age <= 45 && G.children && G.children.length > 0 && !G.mem?.sonderWomanReturnsToWork),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasPhone(G) && (G.age >= 40 && !G.mem?.sonderGrandmotherPhoneCall),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasCar(G) && ((G.character?.country?.name === 'India' || G.character?.country?.name === 'Pakistan' || G.character?.country?.name === 'Bangladesh' || G.character?.country?.name === 'Nepal') && G.age >= 22 && G.age <= 40 && !G.mem?.sonderSouthAsianGroomProcession),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasHealthcare(G) && (G.age >= 20 && !G.mem?.sonderAirportDepartureGate),
...

  {
    ...
    phase: 'adolescence',
    weight: 2,
    when: (G) => hasBooks(G) && (G.age >= 13 && G.age <= 18 && !G.mem?.sonderLibraryLateAfternoon),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => isLiterate(G) && (G.character?.gender === 'female' && G.age >= 30 && (G.character?.country?.archetype === 'subsaharan' || G.character?.country?.archetype === 'developing_urban') && !G.mem?.sonderWomanSmallBusinessLedger),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => isUrban(G) && (G.age >= 40 && G.flags.includes('lost_parent') && !G.mem?.sonderInheritedApartment),
...

  {
    ...
    text: 'The market woman who arrives before dawn to claim her place. She is already there when you pass: the cloth laid down, the goods arranged, the calabash of water. She has done this for twenty years. She knows the light changes over the market in a order. She knows which buyers come first and which come last and what mood each day has by the time the sun reaches the middle stalls. This is a whole life\'s knowledge, and it belongs entirely to her.',
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasCar(G) && ((G.character?.country?.archetype === 'subsaharan' || G.character?.country?.archetype === 'developing_urban') &&
      G.age >= 18 && G.age <= 35 &&
  ...
  // ── STRANGER GLIMPSES: the under-served phases ───────────────────────────────
  // CLAUDE.md asks for a glimpse roughly once per life phase. The pool above is
  // weighted toward young adulthood and midlife; these fill early childhood,
  // childhood, adolescence and late life, and each is anchored to a place or an
  // era rather than floating free of both.

  {
    id: 'sonder_glimpse_ec_carried_past',
...

  {
    ...
    isGlimpse: true,
    isGlimpse: true,
    when: (G) => G.age >= 3 && isHotCountry(G) && !G.mem?.sonderGlimpseEcAsleep,
    ...
    effect: (p) => { p.e += 1; p.setMem('sonderGlimpseEcAsleep', true); },
  },

  {
    id: 'sonder_glimpse_ec_crying_adult',
    phase: 'early_childhood',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 4 && !G.mem?.sonderGlimpseEcCrying,
    text: 'A grown-up you do not know is crying, out in the open, where anyone can see. Someone turns you away from it by the shoulder. You have understood something without being told what it is.',
    effect: (p) => { p.e += 1; p.m -= 1; p.setMem('sonderGlimpseEcCrying', true); },
  },

  {
    id: 'sonder_glimpse_ec_other_child_waiting',
    phase: 'early_childhood',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 4 && hasHealthcare(G) && !G.mem?.sonderGlimpseEcOtherChild,
    text: 'Another child is waiting too, held against a shoulder, facing the way you came in. You look at each other for a long time without either of you doing anything about it. Then a name is called and it is not yours.',
    effect: (p) => { p.e += 1; p.setMem('sonderGlimpseEcOtherChild', true); },
  },

  {
    id: 'sonder_glimpse_ch_shopkeeper_counting',
    phase: 'childhood',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 7 && !G.mem?.sonderGlimpseChShopkeeper,
    text: 'Through the doorway after closing, the shopkeeper is counting the day. He does it twice. His face while he counts is a face he does not use on customers, and you have caught him with it on.',
    effect: (p) => { p.e += 1; p.setMem('sonderGlimpseChShopkeeper', true); },
  },

  {
    id: 'sonder_glimpse_ch_woman_at_the_standpipe',
    phase: 'childhood',
    weight: 2,
    isGlimpse: true,
    isGlimpse: true,
    when: (G) => G.age >= 7 && !hasRunningWater(G) && !G.mem?.sonderGlimpseChStandpipe,
...

  {
    ...
    isGlimpse: true,
    isGlimpse: true,
    when: (G) => G.age >= 8 && isPoor(G) && !G.mem?.sonderGlimpseChBoyWorking,
    ...
    effect: (p) => { p.e += 1; p.m -= 1; p.setMem('sonderGlimpseChBoyWorking', true); },
  },

  {
    id: 'sonder_glimpse_ch_teacher_in_the_street',
    phase: 'childhood',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 7 && wentToSchool(G) && !G.mem?.sonderGlimpseChTeacher,
    text: 'You see a teacher outside of school, carrying shopping, wearing different clothes. She is talking to someone in a voice you have never heard her use. You do not say hello. It would be like speaking to a picture.',
    effect: (p) => { p.e += 2; p.setMem('sonderGlimpseChTeacher', true); },
  },

  {
    ...
    isGlimpse: true,
    isGlimpse: true,
    when: (G) => G.age >= 8 && hasBus(G) && !G.mem?.sonderGlimpseChConductor,
    ...
    effect: (p) => { p.e += 1; p.setMem('sonderGlimpseChConductor', true); },
  },

  {
    id: 'sonder_glimpse_ch_lit_window_walking_back',
    phase: 'childhood',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 8 && hasElectricity(G) && G.season === 'winter' && !G.mem?.sonderGlimpseChLitWindow,
    text: 'Walking back in the dark, you pass a window with the light on and a table laid and nobody sitting at it yet. Somewhere in that house people are about to come and sit down. You keep walking and the window goes behind you.',
    effect: (p) => { p.m += 1; p.e += 1; p.setMem('sonderGlimpseChLitWindow', true); },
  },

  {
    id: 'sonder_glimpse_ad_night_shift_leaving',
    phase: 'adolescence',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 13 && isUrban(G) && G.currentYear >= 1930 && !G.mem?.sonderGlimpseAdNightShift,
  ...
  {
    id: 'sonder_glimpse_ad_studying_late_window',
    phase: 'adolescence',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 13 && hasElectricity(G) && isUrban(G) && !G.mem?.sonderGlimpseAdStudying,
    text: 'A light is on across the way and someone is at a table with books, not moving. They are still there when you look again much later. You do not know what they are studying for or whether they will get it.',
    effect: (p) => { p.e += 2; p.setMem('sonderGlimpseAdStudying', true); },
  },

  {
    id: 'sonder_glimpse_ad_practising_through_wall',
    phase: 'adolescence',
    weight: 2,
    isGlimpse: true,
    isGlimpse: true,
    when: (G) => G.age >= 12 && isUrban(G) && !G.mem?.sonderGlimpseAdPractising,
  ...
  {
    id: 'sonder_glimpse_ad_soldier_at_the_post',
    phase: 'adolescence',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 13 &&
...

  {
    ...
    effect: (p) => { p.e += 1; p.setMem('sonderGlimpseAdOlderGirl', true); },
  },

  {
    id: 'sonder_glimpse_ad_family_eating_outside',
    phase: 'adolescence',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 12 && isHotCountry(G) && !G.mem?.sonderGlimpseAdFamilyOutside,
    text: 'A family is eating outside their door because the room is hotter than the street. They have arranged themselves the way they arrange themselves every evening. You go past close enough to hear one sentence of it, and the sentence is about nothing.',
    effect: (p) => { p.m += 1; p.e += 1; p.setMem('sonderGlimpseAdFamilyOutside', true); },
  },

  {
    id: 'sonder_glimpse_ll_young_couple_arguing',
    phase: 'late_life',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 55 && !G.mem?.sonderGlimpseLlYoungCouple,
    text: 'Two young people are arguing on the corner about something that will not matter to them in four years. You know the shape of it from a distance. You do not stop, and you would not be thanked for stopping.',
    effect: (p) => { p.e += 2; p.setMem('sonderGlimpseLlYoungCouple', true); },
  },

  {
    id: 'sonder_glimpse_ll_nurse_between_rooms',
    phase: 'late_life',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 58 && hasHealthcare(G) && !G.mem?.sonderGlimpseLlNurse,
    text: 'In the corridor a nurse stops between one room and the next and stands still for four or five seconds with her hand flat against the wall. Then she goes into the next room and her face is entirely different. You are the only one who saw the four seconds.',
    effect: (p) => { p.e += 2; p.karma += 1; p.setMem('sonderGlimpseLlNurse', true); },
  },

  {
    id: 'sonder_glimpse_ll_someone_your_age_working',
    phase: 'late_life',
    weight: 2,
    isGlimpse: true,
    isGlimpse: true,
  ...
  {
    id: 'sonder_glimpse_ll_stranger_at_the_grave',
    phase: 'late_life',
    weight: 2,
    isGlimpse: true,
    isGlimpse: true,
...

  {
    ...
    effect: (p) => { p.m += 2; p.setMem('sonderGlimpseLlWalking', true); },
  },

  {
    id: 'sonder_glimpse_ll_new_family_old_house',
    phase: 'late_life',
    weight: 2,
    isGlimpse: true,
    when: (G) => G.age >= 60 && !G.mem?.sonderGlimpseLlNewFamily,
    text: 'People you have never met come out of a door that used to be yours. They have put something different by the entrance and painted over the part that was always chipped. They live there now in the ordinary way, and the house has no memory of you at all.',
    effect: (p) => { p.r += 2; p.e += 1; p.setMem('sonderGlimpseLlNewFamily', true); },
  },

  {
    ...
    effect: (p) => { p.e += 2; p.setMem('sonderGlimpseLlHelped', true); },
  },
