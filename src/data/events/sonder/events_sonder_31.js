...
export const EVENTS_SONDER_31 = [
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => place.isLiterate(G) && (G.age >= 55 && G.age <= 70 && !G.mem?.s31f),
    text: () => pick([
...

  {
    ...
    phase: 'childhood',
    weight: 2,
    when: (G) => place.wentToSchool(G) && (G.age >= 7 && G.age <= 12 && !G.mem?.s31h),
    text: () => pick([
...

{
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => place.hasLeisure(G) && (G.age >= 63 && G.age <= 75 && !G.mem?.s31aa),
    text: () => pick([
