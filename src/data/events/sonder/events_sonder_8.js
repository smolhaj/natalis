...
export const EVENTS_SONDER_8 = [
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => place.hasWeekend(G) && (G.career && G.age >= 22 && !G.mem?.s8WorkMonday),
    text: 'Monday again. You have done this enough times now that Monday has a texture that is different from the other days — not worse, exactly, just specific. The weight of the beginning of the same week.',
    choices: null,
...

  {
    ...
    text: 'Someone asks you what you do and you tell them and they nod and you watch yourself explaining and think: this is what I have done with the years. Not as a complaint. Just as a fact that has a weight when you look at it directly.',
    choices: null,
...

{
...

  {
    ...
    text: 'You have a routine now. You did not decide to have one — it assembled itself out of necessity and then calcified into habit. The order of the morning: the things you do before you do the other things. You do not think about it. It is just the shape of the day.',
    choices: null,
...

{
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => place.hasPhotographs(G) && (G.age >= 40 && !G.mem?.s8HomePhotograph),
...

  {
    ...
    text: 'There is a storm tonight. The particular sound of it — the wind in whatever trees are outside, the quality of the dark — is a sound that will come back to you in other years when you are somewhere else and a storm comes. The first storms lodge.',
    choices: null,
...

  {
    ...
    text: 'The weather on this date always makes you think of something. You do not always know what. A quality of light or temperature that is tied to a year that the current year resembles. The body stores dates by weather.',
    choices: null,
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => place.hasHealthcare(G) && (G.age >= 60 && !G.mem?.s8LateDoctorReg),
    text: 'You see the doctor regularly now in a way you did not before. The appointments have a texture: the waiting room, the numbers, the questions you now know to ask. Your body has become something you manage rather than something you simply inhabit.',
    choices: null,
...

  {
    ...
    text: 'You have lived a decade you did not think would be a decade. Somewhere in your forties you had an implicit sense of what the span of your life would be. You have passed that limit. The years past it are in a register: unexpected, uncalculated, yours to make sense of.',
    choices: null,
...

{
    ...
    text: 'The grandchildren — if you have them — are a kind of time. They age at a rate you can observe, which means you can observe time passing in a way that is harder to see in yourself. Every year the child is taller and you are the same height, which is information.',
    choices: null,
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => place.isUrban(G) && (G.age >= 70 && !G.mem?.s8LateMorningWindow),
