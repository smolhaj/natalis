...

{
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s52d,
    text: 'You keep having a conversation that is, at its core, the same conversation it has always been. Different year, different surface subject, but underneath: the same question, the same difficulty between the same people, the same unresolved thing surfacing through different occasions. You have had this conversation forty times. You recognize it by the third sentence. The recognition does not make it easier to have.',
    choices: null,
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s52l,
    text: 'For the first time, someone else\'s wellbeing was yours to manage. Not temporarily — over time. The experience of having another person\'s welfare depend on decisions you make. The decisions are ordinary: where to go, what to spend, what to say. The ordinary decisions become different when they concern not only you. You are changed by this in ways you only partially understand while it is happening.',
    choices: null,
...

{
    ...
    weight: 2,
    when: (G) => !G.mem?.s52n,
    text: 'There was a rule and nobody explained the reason for it. You were given the rule without the reason. This was the practice — some reasons were too complex to explain, some were lost, some were never examined even by the people who enforced them. You followed the rule. Eventually you understood the reason, or you understood that there was no reason. Both outcomes were possible. Both were informative about the rule.',
    choices: null,
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s52o,
    text: 'It took longer than expected and it is finished now. Looking at it now, from the finished side: it took three years instead of one, it changed shape during, the end is not exactly what the beginning imagined. You are glad it is done and you are aware that what you are glad it is done with is not quite the thing you started. The finished thing and the original intention are in the same category but not the same object.',
    choices: null,
...

{
    ...
    weight: 2,
    when: (G) => !G.mem?.s52q,
    text: 'A slowing down arrives before you name it as slowing down. The recovery that takes a day longer. The stairs that are taken at a different pace. The schedule you rearrange, without consciously rearranging it, toward shorter days. You are not ill. You are not struggling. You are at a different speed than you were, and the adjustment is invisible because it is gradual and internal. You are the last to name it.',
    choices: null,
...

{
    ...
    weight: 2,
    when: (G) => !G.mem?.s52t,
    text: 'Whoever you called when something happened. Not necessarily the person you were closest to in other ways — the person whose response to news was the response you needed. Good news and bad news both. The call or the message. The knowledge that this person would receive what you said in the right way. The relationship was partly constituted by this: the fact of being that person for each other.',
    choices: null,
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s52z,
    text: 'A meal that cannot be replicated. You have tried. The recipe exists, or you have reconstructed it. The ingredients are available. The result is close and not the same, and the gap between close and the same is not a gap in technique. It is a gap in context: the meal was not only the food. It was the specific table and the people and the moment in your lives when you were all at that table together. The thing that made it what it was is not available for recreation.',
    choices: null,
