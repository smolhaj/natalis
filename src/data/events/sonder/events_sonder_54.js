...
import { hasElectricity, hasPhone, hasPhotographs, hasRadio, hasSupermarket, isLiterate, wentToSchool, worksInOffice } from './_sonderGuards.js'
...

{
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => wentToSchool(G) && (!G.mem?.s54b),
...

  {
    ...
    phase: 'late_life',
    weight: 2,
    when: (G) => hasPhotographs(G) && (!G.mem?.s54c),
...

  {
    ...
    phase: 'childhood',
    weight: 2,
    when: (G) => hasRadio(G) && (!G.mem?.s54e),
    text: 'The radio in the next room. Your parents are awake and you are supposed to be asleep. The voices are indistinct. The music starts and stops. The quality of the light under the door is different from daylight — it is the light of the lamp they use in the evenings, the warmth of it. You are not needed there. The house is running without you. This is not loneliness: it is the first evidence that the world has a version of itself that doesn\'t require your presence.',
    choices: null,
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasElectricity(G) && (!G.mem?.s54f),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasPhotographs(G) && (!G.mem?.s54h),
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => isLiterate(G) && (!G.mem?.s54i),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasSupermarket(G) && (!G.mem?.s54n),
...

  {
    ...
    phase: 'childhood',
    weight: 2,
    when: (G) => G.season === 'summer' && (!G.mem?.s54q),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => worksInOffice(G) && (!G.mem?.s54t),
...

  {
    ...
    weight: 2,
    when: (G) => !G.mem?.s54w,
    text: 'A street that no longer exists as it was. The specific configuration of the buildings, the quality of the light when the buildings had that configuration. You walk along what is now there and you are also walking along what was there, which only you and a few other people now carry. This is not grief, exactly. It is the particular position of being the memory of something that no longer exists in any other form.',
    choices: null,
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => hasPhotographs(G) && (!G.mem?.s54aa),
