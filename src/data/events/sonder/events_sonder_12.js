...
import { hasElectricity, hasRadio, hasTV, isLiterate, wentToSchool } from './_sonderGuards.js'
...
export const EVENTS_SONDER_12 = [
...

  {
    ...
    phase: 'childhood',
    weight: 2,
    when: (G) => hasRadio(G) && (G.age >= 7 && G.age <= 14 && !G.mem?.s12SoundRadio),
...

  {
    ...
    weight: 2,
    isGlimpse: true,
    when: (G) => hasTV(G) && (G.age >= 18 && G.age <= 35 && !G.mem?.s12SoundNeighborTv),
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => hasElectricity(G) && ((G.character.country?.archetype === 'subsaharan' || G.character.country?.archetype === 'developing_urban' || G.character.country?.archetype === 'developing_unstable') &&
      G.age >= 15 &&
...

  {
    ...
    text: 'From inside — from a window above the street — you can hear the crowd but not what it is saying. The sound of many voices together before they become individual voices is a frequency. It is lower than you expect. It does not sound like anger. It sounds like a tide. You lean out a little and the words begin to be words.',
    choices: null,
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => wentToSchool(G) && (G.age >= 40 && !G.mem?.s12SoundChildrenNearby),
    text: 'From somewhere outside — the street, the courtyard, the school two blocks away — the sound of children playing. A texture: higher than adult voices, not yet organized into the formal registers of work and negotiation. You are doing something else. You stop for a second. The sound continues. You return to what you were doing.',
    choices: null,
...

  {
    ...
    text: 'You are doing something — folding something, cooking something, cleaning something in a way — and you realize you are doing it the way you were shown. The motion is your parent\'s motion. You are not doing it consciously. It arrived without decision, sometime between watching and practicing. The person who showed you may not be watching now. The motion continues regardless.',
    choices: null,
...

  {
    ...
    phase: 'young_adult',
    weight: 2,
    when: (G) => isLiterate(G) && (G.age >= 18 && !G.mem?.s12OfficialForm),
...

  {
    ...
    text: 'The state radio voice had a cadence — measured, confident, slightly slower than conversational speech, never uncertain. You grew up with it as the voice of official fact. It was authoritative not because it was trusted but because it was omnipresent. You can still hear it exactly, the timbre of that certainty, even now when the station no longer sounds like that.',
    choices: null,
...

  {
    ...
    phase: 'midlife',
    weight: 2,
    when: (G) => isLiterate(G) && ((G.flags.has('revolution_witness') || G.flags.has('post_communist_transition') || G.flags.has('trujillo_generation') || G.political_leaning === 'dissident') &&
      G.age >= 30 &&
...

{
...

  {
    ...
    phase: 'childhood',
    weight: 2,
    when: (G) => hasTV(G) && (G.age >= 8 && G.age <= 16 && !G.mem?.s12HourSunday),
