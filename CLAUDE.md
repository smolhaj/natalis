...
- `src/data/events_followthrough_5.js` — 8 flag follow-through events for flags set by BUILD 40 and other orphaned flags: civil rights generation legacy (midlife), resistance through art recognized (late_life), art shown late exists (midlife), compromised ledger reckoning (late_life, 2-choice), censored journalist story (midlife), intimidated body reflex (midlife), independence generation reckoning (late_life), first coup not last (midlife)
...
*`events_followthrough_5.js`* — 8 follow-through events for flags set by BUILD 40 and other orphaned flags: `civil_rights_generation` → legacy reflection (midlife, once); `resistance_through_art` → recognized by a younger person (late_life age 60+, once); `art_shown_late` → work exists in the world now (midlife, once); `compromised` → late-life ledger reckoning (2-choice: name it / justify it); `censored_journalist` → the story that ran later (midlife, once); `intimidated_into_silence` → body-memory reflex in a meeting (midlife, once); `independence_generation_self` → accounting at 55+ (late_life, once); `first_coup_witness` → it was not the last (midlife, once).
